import { db } from "@/db";
import { roadshowTrips, roadshowLegs, roadshowMeetings } from "@/db/schema";

async function seed() {
  console.log("🌱 Seeding roadshow trip data...");

  // Clear existing roadshow data
  await db.delete(roadshowMeetings);
  await db.delete(roadshowLegs);
  await db.delete(roadshowTrips);

  // --- TRIP ---
  const [trip] = await db
    .insert(roadshowTrips)
    .values({
      ...
      fundThesis: `Current Equities is a dedicated infrastructure PE vehicle focused on behind-the-meter energy for AI data centers. We invest in the physical layer — power generation, energy campus development, and deployment infrastructure — that makes intelligence possible.
...
AIP case study: Jerry invested at AIP Series A. Nscale acquired AIP at $1.5B (5x in <1 year). Microsoft LOI for 1.35 GW. This is the model we replicate.`,
  ...
  await db.insert(roadshowMeetings).values(meetingsData);
  console.log(`  ✅ ${meetingsData.length} meetings created`);
  console.log("🎉 Roadshow seed complete!");
}
