/**
 * System prompt for one-click recon generation.
 * Claude produces structured JSON with all sections.
 */
export function getGeneratePrompt(reconContext: string): string {
  return `You are preparing a strategic recon brief for a senior PE/VC fund principal. This is high-stakes work — the analysis must be thorough, strategic, and actionable.

You have full CRM, meeting, and project context below. Use it to generate a comprehensive strategic brief with the following sections:

1. **Intel Summary** — 3-5 paragraph dossier on the target: who they are, what they do, their scale, recent news/activity, strategic position in their market. Be specific — use the data provided, don't generalize.
...
2. **Positioning Analysis** — Two parts:
   ...
   - "Strategic Angle": The one-liner positioning — why this matters and what the ideal outcome is.

3. **Playbook** — A conversational script/game plan, broken into 6 sections:
...
4. **Prep Checklist** — 5-8 actionable pre-engagement tasks (research to do, materials to prepare, people to brief, talking points to rehearse).
...
export function getRefinePrompt(
  ...
  return `You are refining a section of a strategic recon brief for a senior PE/VC fund principal.
